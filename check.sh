...
if [[ -f "$INSTALL_STATE" ]]; then
    ...
    # Не проблема: setup.sh умеет вычитывать настройки с самого сервера,
    # а файл создаётся при первой же правке через меню.
    info "нет $INSTALL_STATE — нода ставилась ранней версией"
    info "setup.sh подставит настройки, вычитав их с сервера; файл появится после первой правки через меню"
fi
